...
public sealed partial class Queen
{
    ...
    /// <summary>Whether this mission's raw answer is worth spending a synthesis call on.</summary>
    public static bool ShouldSynthesizeAnswer(string rawAnswer) =>
        AnthillRuntime.EnableAnswerSynthesis
    ...
    /// <summary>
    /// v2.16.0: produce the plain-English answer stored in <c>Mission.FinalResult</c>.
    ///
    /// <c>UserResult</c> keeps the raw best-task output and <c>DebugResult</c> keeps the full
    /// trace, so nothing is replaced or lost — this only changes what is shown FIRST. Every
    /// failure path falls back to the raw answer: no router, feature disabled, short answer,
    /// provider down, or an empty response.
    ///
    /// Routed under the "scribe" role, which resolves through the normal route table (unknown
    /// roles fall back), so the model that writes answers can be pointed somewhere cheap in
    /// Settings → Model Routing without touching code.
    /// </summary>
    public string ComposeFinalAnswer(Mission mission)
    {
        var raw = mission.UserResult ?? "";
        if (Router is null || !ShouldSynthesizeAnswer(raw)) return raw;

        string? synthesized = null;
        try
        {
            synthesized = Router.Generate("scribe", BuildAnswerSynthesisPrompt(mission, raw),
                mission.Id, antName: "scribe");
        }
        catch (Exception ex)
        {
            // Synthesis is a presentation nicety. It must never be able to fail a finished mission.
            Memory.LogEvent(mission.Id, "answer_synthesis_failed",
