...
// only two that Infrastructure and Integrations actually use (56 and 10 call sites) — so they are
...
// no core imports at all — and they are SHARED: shadow mode is in the core, the infrastructure is a
...
// v3.8.7 — the infrastructure left the core. The API is a composition root, so it may name the module;
// Anthill.Core may not, and its own tests never load it.
global using Anthill.Modules.Infrastructure;
#if MICROMOUND
