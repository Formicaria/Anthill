/**
 * The infrastructure domain, lifted out of app.js. v0.3.8.52 — AUTONOMY-10, the app.js split.
 *
 ...
 * time is the PAGE_ENTER['infrastructure'] registration below, and PAGE_ENTER is defined in app.js.
...
// -- Infrastructure Inventory (v1.10.0, NORTH_STAR Phase 6) ----------------------------
let HL_HOSTS=[], HL_SVCS=[], HL_DEPS=[];
PAGE_ENTER['infrastructure']=()=>loadInfrastructure();
...
async function hlLoadAutomation(){
  ...
  try{
    const [r,runs]=await Promise.all([api('/infrastructure/automation/rules'),api('/infrastructure/automation/runs')]);
    ...
    if(st) st.textContent=d.enabled?'engine ON · rules fire only when individually enabled':'engine OFF (infrastructure_automation_enabled) — rules are stored but nothing evaluates';
    body.innerHTML=!rules.length
      ? '<span style="color:var(--dim)">No rules yet. POST /infrastructure/automation/rules — e.g. disk_above_percent → warn_event, service_down → propose_restart (restart is approval-gated, never direct).</span>'
...
async function hlAutoToggle(id,on){
  const r=await window.api('/infrastructure/automation/rules/'+encodeURIComponent(id)+'/'+(on?'enable':'disable'),'POST');
...
async function hlAutoEvaluate(){
  const r=await window.api('/infrastructure/automation/evaluate','POST');
...
async function hlLoadBackup(){
  ...
  try{
    const r=await api('/infrastructure/backup/coverage');
...
async function hlRunbook(kind,id){
  ...
  try{
    const r=await api('/infrastructure/backup/runbook/'+encodeURIComponent(kind)+'/'+encodeURIComponent(id));
...
async function loadInfrastructure(){
  ...
  try{
    const [sum,hosts,svcs,deps,chg]=await Promise.all([
      api('/infrastructure/summary'),api('/infrastructure/hosts'),api('/infrastructure/services'),
      api('/infrastructure/dependencies'),api('/infrastructure/changes')]);
...
function renderHlSummary(d){
  ...
  el.innerHTML=
    ...
    '<br>Providers: '+provs+
    '<br><span style="color:var(--dim)">Read-only foundation - actions arrive approval-gated in V2.1. Toggles live in config (infrastructure_enabled, infrastructure_scheduler_enabled, infrastructure_mock_providers_enabled).</span>';
}
...
async function loadHlVirt(){
  try{
    const [st,vms,cts,pools]=await Promise.all([
      api('/infrastructure/virtualization/status'),api('/infrastructure/vms'),api('/infrastructure/containers'),api('/infrastructure/storage')]);
...
function renderHlVirtConns(data){
  ...
  let bar='<div style="grid-column:1/-1;display:flex;align-items:center;gap:10px;flex-wrap:wrap;padding:8px 10px;border:1px solid var(--border);border-radius:6px;font-size:11px;">'+
    '<span>Infrastructure subsystem: <b style="color:'+(data.infrastructure_enabled?'var(--green)':'var(--red)')+'">'+(data.infrastructure_enabled?'enabled':'disabled')+'</b></span>'+
    ...
    (admin?('<span style="flex:1"></span>'+
      '<button class="btn btn-ghost" data-onclick="hlSetInfrastructureGate(\'infrastructure_enabled\','+(!data.infrastructure_enabled)+')">'+(data.infrastructure_enabled?'Disable subsystem':'Enable subsystem')+'</button>'+
      '<button class="btn btn-ghost" data-onclick="hlSetInfrastructureGate(\'infrastructure_scheduler_enabled\','+(!data.scheduler_enabled)+')">'+(data.scheduler_enabled?'Disable scheduler':'Enable scheduler')+'</button>'+
...
async function hlAllowVirtHost(k){
  ...
  const r=await api('/infrastructure/allowlist','POST',{target:host,note:k+' virtualization'});
  hlMsg(r.success?('Host '+host+' allowlisted — now Sync now to pull inventory.'):(r.message||'Failed'),r.success);
  if(r.success) loadHlVirt();
}

async function hlSetInfrastructureGate(key,val){
...
async function hlSaveVirtConn(k){
  ...
  if(!g('host')) return;
  const payload={};
  payload['infrastructure_'+k+'_enabled']=g('enabled').checked;
  payload['infrastructure_'+k+'_host']=g('host').value.trim();
  const port=parseInt(g('port').value,10); if(port>0) payload['infrastructure_'+k+'_port']=port;
  payload['infrastructure_'+k+'_credential_id']=g('credid').value.trim();
  payload['infrastructure_'+k+'_insecure_tls']=g('insecure').checked;
  const r=await api('/settings','POST',payload);
...
async function hlSaveVirtCred(k){
  ...
  const r=await api('/infrastructure/credentials','POST',{id:id,kind:k,target_host:(g('host')?g('host').value.trim():''),secret:secret});
...
async function hlSyncVirt(k){
  ...
  const r=await api('/infrastructure/virtualization/'+encodeURIComponent(k)+'/sync','POST',{});
  if(!r.success){ hlMsg(r.message||'Failed',false); return; }
  loadInfrastructure(); // full refresh: node graph + inventory tables (loadInfrastructure calls loadHlVirt)
  ...
  try{
    const st=await api('/infrastructure/virtualization/status');
...
async function hlPveSync(){
  ...
  const r=await api('/infrastructure/proxmox/sync','POST',{});
  hlMsg(r.message||(r.success?'Synced':'Failed'),r.success);
  if(r.success){ loadInfrastructure(); }
}
...
async function loadHlDashboard(){
  try{
    const r=await api('/infrastructure/dashboard');
...
async function loadHlIncidents(){
  try{
    const r=await api('/infrastructure/incidents');
...
async function hlIncDetail(id,title){
  ...
  try{
    const [tl,sim]=await Promise.all([
      api('/infrastructure/incidents/'+encodeURIComponent(id)+'/timeline'),
      api('/infrastructure/incidents/'+encodeURIComponent(id)+'/similar')]);
...
async function hlOpenIncident(){
  ...
  const r=await api('/infrastructure/incidents','POST',{title, subjectKind:'manual', subjectId:title, severity:'warning'});
...
async function hlIncStatus(status){
  ...
  const r=await api('/infrastructure/incidents/'+encodeURIComponent(HL_INC_OPEN)+'/status','POST',{status, rootCause:root});
...
async function loadHlActions(){
  try{
    const r=await api('/infrastructure/actions');
    if(!r.success){ return; }
    const d=r.data||{};
    ...
    if(st) st.innerHTML=HL_ACT_STOPPED?athPill('danger','INFRASTRUCTURE_STOP ENGAGED'):athPill('ok','actions armed');
...
function renderHlActions(items){
  ...
  tb.innerHTML=items.slice(0,25).map(a=>{
    ...
    if(a.state==='approved')
      ops+='<button class="btn btn-primary" data-onclick="hlActExecute(\''+jsArg(a.approvable_id)+'\')"'+(HL_ACT_STOPPED?' disabled title="INFRASTRUCTURE_STOP is engaged"':'')+'>▶ Execute</button>';
...
async function hlActPropose(){
  ...
  const r=await api('/infrastructure/actions/propose','POST',{
...
async function hlActOp(id,verb){
  const r=await api('/infrastructure/actions/'+encodeURIComponent(id)+'/'+verb,'POST',{});
...
async function hlActExecute(id){
  ...
  const r=await api('/infrastructure/actions/'+encodeURIComponent(id)+'/execute','POST',{});
  hlMsg(r.message||(r.success?'Executed':'Refused'),r.success);
  loadHlActions();
}
async function hlActKillSwitch(){
  if(HL_ACT_STOPPED){
    if(!await uiConfirm('Clear INFRASTRUCTURE_STOP? Approved actions will be executable again.')) return;
    const r=await api('/infrastructure/actions/resume','POST',{});
    hlMsg(r.message||(r.success?'Resumed':'Refused'),r.success);
  }else{
    const r=await api('/infrastructure/actions/stop','POST',{reason:'console kill switch'});
...
// ---- v2.5.3 Console Refit R3: infrastructure category sub-pages ---------------------------------------
// Every card on the Infrastructure page declares exactly ONE home via data-hlsub; the sub-nav filters
// visibility. Cards without data-hlsub (entity detail, incident detail, + Add / Manage drawer)
// are on-demand overlays and stay available from every sub-page. Keyboard: g h opens Infrastructure,
...
function hlSubShow(name,fromRoute){
  ...
  document.querySelectorAll('#page-infrastructure [data-hlsub]').forEach(c=>{ c.style.display=(c.dataset.hlsub===name)?'':'none'; });
  document.querySelectorAll('#hl-subnav .hl-sub-btn').forEach(b=>b.classList.toggle('active',b.dataset.sub===name));
  // v2.24.0: the shadow panel loads with its sub-page rather than on every infrastructure open — it joins
  ...
  if(!fromRoute && typeof HLSUB_ROUTE!=='undefined' && document.getElementById('page-infrastructure')?.classList.contains('active')){
    const r=HLSUB_ROUTE[name];
    if(r && typeof updateChrome==='function'){ updateChrome(r,'infrastructure'); try{ history.replaceState(null,'','#'+r); }catch(e){} }
  }
}
...
async function renderHlDeck(){
  ...
  try{
    const [v,c,h,m]=await Promise.all([api('/infrastructure/vms'),api('/infrastructure/containers'),api('/infrastructure/health/results'),api('/infrastructure/metrics/nodes')]);
  ...
  // and vm.node_id / ct.node_id reference infrastructure_nodes.id (e.g. "pve-node:host:pve1").
...
async function hl3GuestPage(kind,id){
  ...
  try{
    const ev=await api('/infrastructure/events');
...
async function renderHl3Apps(){
  ...
  let items=[];
  try{ const r=await api('/infrastructure/arr'); items=(r&&r.success&&r.data&&Array.isArray(r.data.items))?r.data.items:[]; }catch(e){}
  HL3.apps=items;
...
async function hl3ArrAdd(){
  ...
  const r=await api('/infrastructure/arr','POST',{kind,name:document.getElementById('hl3-ar-name').value.trim()||kind,url,apiKey:key});
...
async function hl3ArrSync(){
  const r=await api('/infrastructure/arr/sync','POST',{});
...
async function hl3ArrRemove(id){
  ...
  const r=await api('/infrastructure/arr/'+encodeURIComponent(id),'DELETE');
...
// Data source: GET /infrastructure/integrations/{id}/widgets/{kind} (integration_state + freshness).
const WIDGET_KINDS={
...
function wgtPath(st){ return '/infrastructure/integrations/'+encodeURIComponent(st.integrationId)+'/widgets/'+encodeURIComponent(st.kind); }
...
/* THE WIDGET ZONE THIS PAGE USED TO BE CALLED. v0.3.8.128.

   `uiState.widgets` is keyed by zone id and this page's zone was `homelab`. The CHANGELOG records
   the analogous case and the decision taken then: the Agent Inspector became the Ant Inspector and
   "the widget id stays `agent-inspector`: it is persisted dashboard-layout state, and renaming it
   would silently drop every saved placement of that card." The same is true here, so the rename is
   paired with the read that makes it survivable rather than done and hoped about.

   Runs once per render before the zone is read. Idempotent: after the first save there is no
   `homelab` key left to find. A document carrying both keeps the current one — an operator who has
   already arranged this page since upgrading should not have that undone by a leftover. */
function wgtZoneMigrateLegacy(){
  const w=uiState.widgets; if(!w||typeof w!=='object') return;
  if(Array.isArray(w.homelab)){
    if(!Array.isArray(w.infrastructure)) w.infrastructure=w.homelab;
    delete w.homelab;
    saveUiState();
  }
}
...
async function renderHl3Widgets(){
  ...
  wgtZoneMigrateLegacy();
  const layout=wgtZone('infrastructure');
...
function hl3WidgetMove(btn,dir){
  ...
  const list=wgtZone('infrastructure').slice();
  ...
  wgtZoneSave('infrastructure',list); renderHl3Widgets();
}
function hl3WidgetRemove(btn){
  const el=btn.closest('.wgt'); if(!el) return;
  wgtZoneSave('infrastructure',wgtZone('infrastructure').filter(e=>e.id!==el.dataset.wid));
  renderHl3Widgets();
}
...
function hl3WidgetAdd(){
  ...
  wgtZoneSave('infrastructure',list);
...
function renderHlTargets(){
  ...
  collectionManager({
    el,
    load:async()=>{
      const r=await api('/infrastructure/allowlist');
    ...
    rowActions:[
      {icon:'✎',title:'Edit note',run:async e=>{
        ...
        if(note!==null&&note!==undefined) await api('/infrastructure/allowlist/'+encodeURIComponent(e.id),'PUT',{note:note});
      }},
      {icon:e=>e.kind==='deny'?'✔':'⛔',title:'Flip allow/deny',run:async e=>{
        await api('/infrastructure/allowlist/'+encodeURIComponent(e.id),'PUT',{kind:e.kind==='deny'?'allow':'deny'});
      }},
      {icon:e=>e.enabled?'⏸':'▶',title:'Enable/disable',run:async e=>{
        await api('/infrastructure/allowlist/'+encodeURIComponent(e.id),'PUT',{enabled:!e.enabled});
      }},
      ...
      {icon:'<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:-1px"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg>',title:'Remove entry',run:async e=>{
        if(await uiConfirm('Remove target \''+e.target+'\'?')) await api('/infrastructure/allowlist/'+encodeURIComponent(e.id),'DELETE');
      }},
    ],
    bulk:[
      {label:'Enable',run:ids=>api('/infrastructure/allowlist/bulk','POST',{action:'enable',ids})},
      {label:'Disable',run:ids=>api('/infrastructure/allowlist/bulk','POST',{action:'disable',ids})},
      {label:'Remove',confirm:'Remove {n} target entr(y/ies)?',run:ids=>api('/infrastructure/allowlist/bulk','POST',{action:'remove',ids})},
    ],
  });
}
async function hlTargetAdd(){
  ...
  const r=await api('/infrastructure/allowlist','POST',{target,kind,note});
  hlMsg(r.message||(r.success?'Saved':'Failed'),r.success);
...
async function loadHlRisks(){
  try{
    const [devs,risks]=await Promise.all([api('/infrastructure/devices'),api('/infrastructure/risks')]);
...
async function hlAddDevice(){
  ...
  const r=await api('/infrastructure/devices','POST',{name, kind:document.getElementById('hl-n-kind').value, mac,
...
async function hlDelDevice(id){
  const r=await api('/infrastructure/devices/'+encodeURIComponent(id),'DELETE');
  hlMsg(r.message||(r.success?'Removed':'Failed'),r.success);
...
async function hlAnalyzeRisks(){
  ...
  const r=await api('/infrastructure/risks/analyze','POST',{});
  hlMsg(r.message||(r.success?'Done':'Failed'),r.success);
  if(r.success) loadHlRisks();
}

async function hlAckRisk(id){
  const r=await api('/infrastructure/risks/'+encodeURIComponent(id)+'/ack','POST',{});
...
async function loadHlHealth(){
  try{
    const [sum,sch,res]=await Promise.all([
      api('/infrastructure/health/summary'),api('/infrastructure/health/schedules'),api('/infrastructure/health/results')]);
...
async function hlAddCheck(){
  ...
  const r=await api('/infrastructure/health/schedules','POST',{checkKind:document.getElementById('hl-c-kind').value,
...
async function hlDelCheck(id){
  const r=await api('/infrastructure/health/schedules/'+encodeURIComponent(id),'DELETE');
  hlMsg(r.message||(r.success?'Removed':'Failed'),r.success);
...
async function hlRunChecks(){
  ...
  const r=await api('/infrastructure/health/run','POST',{});
  hlMsg(r.message||(r.success?'Done':'Failed'),r.success);
  loadHlHealth();
}

async function hlTestNotify(){
  const r=await api('/infrastructure/notifications/test','POST',{});
...
async function hlAddHost(){
  ...
  const r=await api('/infrastructure/hosts','POST',{name, kind:document.getElementById('hl-h-kind').value,
    address:document.getElementById('hl-h-addr').value.trim(), os:document.getElementById('hl-h-os').value.trim(), roleTags:tags});
  hlMsg(r.message||(r.success?'Saved':'Failed'),r.success);
  if(r.success){ ['hl-h-name','hl-h-addr','hl-h-os','hl-h-tags'].forEach(id=>document.getElementById(id).value=''); loadInfrastructure(); }
}
...
async function hlAddService(){
  ...
  const r=await api('/infrastructure/services','POST',{name, nodeId:document.getElementById('hl-s-node').value,
  ...
  hlMsg(r.message||(r.success?'Saved':'Failed'),r.success);
  if(r.success){ ['hl-s-name','hl-s-url','hl-s-ports','hl-s-owner'].forEach(id=>document.getElementById(id).value=''); loadInfrastructure(); }
}
...
async function hlAddDep(){
  ...
  const r=await api('/infrastructure/dependencies','POST',{fromKind:fk, fromId:fid, toKind:tk, toId:tid,
    dependencyKind:document.getElementById('hl-d-kind').value, notes:document.getElementById('hl-d-notes').value.trim()});
  hlMsg(r.message||(r.success?'Saved':'Failed'),r.success);
  if(r.success){ document.getElementById('hl-d-notes').value=''; loadInfrastructure(); }
}

async function hlDelDep(id){
  const r=await api('/infrastructure/dependencies/'+encodeURIComponent(id),'DELETE');
  hlMsg(r.message||(r.success?'Removed':'Failed'),r.success);
  if(r.success) loadInfrastructure();
}

async function hlExport(){
  const r=await api('/infrastructure/export');
...
async function hlImportFile(ev){
  ...
  try{
    ...
    const r=await api('/infrastructure/import','POST',bundle);
