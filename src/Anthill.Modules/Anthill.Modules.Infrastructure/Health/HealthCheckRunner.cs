...
namespace Anthill.Modules.Infrastructure.Health;

/// <summary>
/// Deterministic health-check execution (v1.11.0, NORTH_STAR Phase 7). Plain C# — never routed
/// through the model router. Every check: (1) resolves its target host, (2) must pass the Infrastructure
...
public sealed class HealthCheckRunner
{
    ...
    public HealthCheckRunner(IInfrastructureRepository repository, IInfrastructureTargetGuard targetGuard, NotificationService? notifier = null)
    {
    ...
    private static TimeSpan TimeoutFor(HealthCheckSchedule s) =>
        TimeSpan.FromMilliseconds(s.TimeoutMs > 0 ? Math.Clamp(s.TimeoutMs, 250, 60_000) : InfrastructureRuntime.Options.HealthTimeoutMs);
    ...
    public async System.Threading.Tasks.Task<InfrastructureProviderResult> RunAllAsync(CancellationToken ct)
    {
        ...
        return failed == 0
            ? InfrastructureProviderResult.Success($"{schedules.Count} check(s) run", schedules.Count)
            : InfrastructureProviderResult.Failure($"{failed}/{schedules.Count} check(s) failed");
    }
    ...
    public async System.Threading.Tasks.Task<HealthCheckResult> RunCheckAsync(HealthCheckSchedule schedule, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            else if (!_targetGuard.IsAllowed(host))
            {
                ...
                result.Status = HealthStatus.Failed;
                result.Detail = $"host '{host}' is not on the infrastructure target allowlist — add it under /infrastructure/allowlist";
            }
            else
            {
