...
/// <summary>
/// SQLite persistence for the infrastructure foundation (v1.9.0, NORTH_STAR Phase 4). Lives in the same
/// database file as colony memory so infrastructure knowledge is linkable to missions and searchable, but
...
/// </summary>
public sealed partial class InfrastructureRepository : IInfrastructureRepository, IDisposable
{
    public string DbPath { get; }
    private readonly object _writeLock = new();

    /// <summary>
    /// Where <see cref="RecordEvent"/> announces what it just recorded. v3.8.7.
    ///
    /// The infrastructure has always had its own event stream — the <c>infrastructure_events</c> table, nineteen
    /// call sites, its own severity vocabulary — and, exactly like <c>SqliteMemory.LogEvent</c>
    /// before v3.8.3, it had no live outlet. A VM restarting, a credential being used, an inventory
    /// drifting: all recorded, none of it visible on the console's event stream. An operator
    /// watching a infrastructure action run saw nothing until they went looking in a different panel.
    ///
    /// Same retrofit, same ordering guarantee, same default. Never null — it starts as the no-op
    /// bus, so an unwired repository behaves exactly as it did before this property existed, which
    /// is what every one of the infrastructure tests relies on.
    /// </summary>
    public IEventBus EventBus { get; set; } = NullEventBus.Instance;

    public InfrastructureRepository(string? dbPath = null)
    {
        // v3.8.7: the path arrives already resolved. The module does not know where the script
        // directory is, and working it out would mean reading the core's runtime.
        var raw = dbPath ?? InfrastructureRuntime.Options.DatabasePath;
    ...
    /// <summary>The 15 NORTH_STAR Phase 4 tables plus infrastructure_meta (scheduler job state, D4).</summary>
    internal static readonly string[] TableNames =
    {
        "infrastructure_nodes", "network_devices", "services", "vm_inventory", "container_inventory",
        "storage_inventory", "backup_inventory", "health_checks", "infrastructure_events", "change_log",
        "incidents", "dependencies", "risk_records", "infrastructure_credentials", "infrastructure_target_allowlist",
    ...
    private static readonly string[] SchemaStatements =
    {
        ...
        @"CREATE TABLE IF NOT EXISTS infrastructure_target_allowlist (
        ...
        @"CREATE INDEX IF NOT EXISTS idx_infrastructure_events_created ON infrastructure_events(created_at)",
    ...
    /* THE FIVE TABLES THIS SUBSYSTEM USED TO CALL ITSELF. v0.3.8.128.

       `homelab_nodes`, `homelab_events`, `homelab_credentials`, `homelab_target_allowlist` and
       `homelab_meta` are `infrastructure_*` now. Every other table in this module was never
       prefixed and is untouched.

       ORDER IS THE WHOLE CORRECTNESS ARGUMENT. This runs BEFORE the `CREATE TABLE IF NOT EXISTS`
       statements, because those are idempotent in the worst possible way here: run them first on a
       pre-rename database and SQLite happily creates five EMPTY `infrastructure_*` tables beside
       five populated `homelab_*` ones, the rename below then finds its destination occupied and
       does nothing, and the colony comes up with an inventory, a credential store and an allowlist
       that are all silently empty. Nothing errors. The operator's hosts are simply gone.

       `ALTER TABLE ... RENAME TO` carries indexes and data with it. The guard is presence of the
       old table AND absence of the new one — the only state in which a rename is unambiguous. A
       database with both (a half-finished earlier attempt, or a restored backup laid over a newer
       file) is left exactly as it is: this refuses rather than guessing which one is the operator's
       real data, and the `CREATE ... IF NOT EXISTS` below then leaves the new one in place.

       No schema-version row, deliberately. This module has never had one and adding a ledger to
       carry a single rename would be a second migration system beside `schema_migrations` in core.
       The rename is self-describing: once done, the old table does not exist and this is a no-op
       forever. */
    private static void RenameLegacyHomelabTables(SqliteConnection conn, SqliteTransaction tx)
    {
        foreach (var (legacy, current) in new[]
                 {
                     ("homelab_nodes", "infrastructure_nodes"),
                     ("homelab_events", "infrastructure_events"),
                     ("homelab_credentials", "infrastructure_credentials"),
                     ("homelab_target_allowlist", "infrastructure_target_allowlist"),
                     ("homelab_meta", "infrastructure_meta"),
                 })
        {
            if (!TableExists(conn, tx, legacy) || TableExists(conn, tx, current)) continue;
            using var cmd = conn.CreateCommand();
            cmd.Transaction = tx;
            // Identifiers are literals from the array above, never operator input.
            cmd.CommandText = $"ALTER TABLE {legacy} RENAME TO {current}";
            cmd.ExecuteNonQuery();
        }

        // The one index that carried the old name. Renaming a table does not rename its indexes,
        // and a stale `idx_homelab_events_created` would sit beside the new one the DDL creates —
        // harmless, but it is exactly the kind of leftover that makes a later reader wonder whether
        // the migration finished.
        if (TableExists(conn, tx, "infrastructure_events"))
        {
            using var drop = conn.CreateCommand();
            drop.Transaction = tx;
            drop.CommandText = "DROP INDEX IF EXISTS idx_homelab_events_created";
    ...
    private void InitDb()
    {
        lock (_writeLock)
        {
            ...
            RenameLegacyHomelabTables(conn, tx);   // v0.3.8.128 — BEFORE the DDL below. See remarks.
            ...
            EnsureColumn(conn, tx, "infrastructure_target_allowlist", "list_kind", "TEXT NOT NULL DEFAULT 'allow'");
    ...
    public void UpsertNode(InfrastructureNode node, string changedBy)
    {
        ...
        lock (_writeLock)
        {
            using var conn = Connect();
            using var cmd = conn.CreateCommand();
            cmd.CommandText = @"INSERT INTO infrastructure_nodes (id, name, kind, address, os, role_tags_json, notes, created_at, updated_at)
    ...
    public IReadOnlyList<InfrastructureNode> ListNodes()
    {
        var list = new List<InfrastructureNode>();
        using var conn = Connect();
        using var cmd = conn.CreateCommand();
        cmd.CommandText = "SELECT id, name, kind, address, os, role_tags_json, notes, created_at, updated_at FROM infrastructure_nodes ORDER BY name";
        using var r = cmd.ExecuteReader();
        while (r.Read())
        {
            list.Add(new InfrastructureNode
            {
    ...
    public void OpenIncident(IncidentRecord incident, string openedBy)
    {
        ...
        RecordEvent(new InfrastructureEvent
        {
    ...
    /// <summary>
    /// Record a infrastructure event durably, then announce it. v3.8.7 adds the second half.
    ///
    /// Persist-then-publish, never the reverse, and never inside the write lock — the same ordering
    /// <c>SqliteMemory.LogEvent</c> keeps, for the same reason: a subscriber must not be able to act
    /// on an event that a subsequent database failure leaves unrecorded.
    ///
    /// One wrinkle the mission log does not have. This insert is <c>OR IGNORE</c>, because providers
    /// use stable ids (<c>pve-task:&lt;UPID&gt;</c>) so a re-sync re-offers events already stored.
    /// So the row count decides whether to publish: a duplicate silently changes nothing in the
    /// table, and announcing it anyway would make every Proxmox re-sync replay the whole recent
    /// history onto the console — the stream would fill with events that did not just happen.
    /// </summary>
    public void RecordEvent(InfrastructureEvent evt)
    {
        ...
        lock (_writeLock)
        {
            ...
            cmd.CommandText = @"INSERT OR IGNORE INTO infrastructure_events (id, event_type, subject_kind, subject_id, severity, message, mission_id, created_at)
        ...
        if (written > 0) EventBus.Publish(ToColonyEvent(evt));
    }

    /// <summary>
    /// A infrastructure event, as the colony sees it.
    ///
    /// The type is PREFIXED rather than passed through. Infrastructure types are drawn from a different
    /// vocabulary — <c>scheduler_job</c>, <c>credential_used</c>, <c>inventory_changed</c> — and a
    /// console filtering on a bare name would silently mix infrastructure activity into mission
    /// panels the first time the two vocabularies happened to agree on a word. <c>infrastructure_</c> keeps
    /// them tellable apart by inspection, and the original type stays in the metadata for anything
    /// that wants to group by it.
    ///
    /// <c>MissionId</c> is frequently empty here, and that is correct: most infrastructure events are not
    /// caused by a mission. A drifting inventory belongs to the colony, not to a task.
    /// </summary>
    private static ColonyEvent ToColonyEvent(InfrastructureEvent evt) => new()
    {
        Id = evt.Id,
        EventType = "infrastructure_" + evt.EventType,
        MissionId = evt.MissionId,
        Message = evt.Message,
        CreatedAt = AnthillTime.ParseIsoOrNow(evt.CreatedAt),
        Metadata = new Dictionary<string, object?>
        {
            ["infrastructure_event_type"] = evt.EventType,
            ["subject_kind"] = evt.SubjectKind,
            ["subject_id"] = evt.SubjectId,
            ["severity"] = evt.Severity,
        },
    };

    public IReadOnlyList<InfrastructureEvent> RecentEvents(int limit = 50)
    {
        var list = new List<InfrastructureEvent>();
        using var conn = Connect();
        using var cmd = conn.CreateCommand();
        cmd.CommandText = "SELECT id, event_type, subject_kind, subject_id, severity, message, mission_id, created_at FROM infrastructure_events ORDER BY created_at DESC LIMIT $limit";
        Bind(cmd, "$limit", Math.Clamp(limit, 1, 500));
        using var r = cmd.ExecuteReader();
        while (r.Read())
        {
            list.Add(new InfrastructureEvent
            {
    ...
    public void UpsertHealthSchedule(Anthill.Modules.Infrastructure.Health.HealthCheckSchedule schedule, string changedBy)
    {
    ...
    public IReadOnlyList<Anthill.Modules.Infrastructure.Health.HealthCheckSchedule> ListHealthSchedules()
    {
        var list = new List<Anthill.Modules.Infrastructure.Health.HealthCheckSchedule>();
        using var conn = Connect();
        using var cmd = conn.CreateCommand();
        ...
        while (r.Read())
        {
            list.Add(new Anthill.Modules.Infrastructure.Health.HealthCheckSchedule
            {
                Id = r.GetString(0), CheckKind = r.GetString(1), Target = r.GetString(2),
                ServiceId = r.IsDBNull(3) ? "" : r.GetString(3), NodeId = r.IsDBNull(4) ? "" : r.GetString(4),
    ...
    public InfrastructureInventoryExport ExportInventory() => new()
    {
        ExportedAt = AnthillTime.NowUtc().ToIso(),
        AnthillVersion = InfrastructureRuntime.Options.ColonyVersion,
    ...
    /// </summary>
    public (int Nodes, int Services, int Dependencies) ImportInventory(InfrastructureInventoryExport bundle, string importedBy)
    {
    ...
    public void AddAllowlistEntry(TargetAllowlistRecord entry)
    {
        ...
        lock (_writeLock)
        {
            using var conn = Connect();
            using var cmd = conn.CreateCommand();
            cmd.CommandText = @"INSERT INTO infrastructure_target_allowlist (id, target, list_kind, note, enabled, added_by, created_at)
    ...
    public int SetAllowlistEnabled(IReadOnlyCollection<string> ids, bool enabled, string changedBy)
    {
        ...
        lock (_writeLock)
        {
            ...
            {
                using var cmd = conn.CreateCommand();
                cmd.CommandText = "UPDATE infrastructure_target_allowlist SET enabled=$en WHERE id=$id";
    ...
    public int RemoveAllowlistEntries(IReadOnlyCollection<string> ids, string removedBy)
    {
        ...
        lock (_writeLock)
        {
            ...
            {
                using var cmd = conn.CreateCommand();
                cmd.CommandText = "DELETE FROM infrastructure_target_allowlist WHERE id=$id";
                Bind(cmd, "$id", id);
    ...
    public IReadOnlyList<TargetAllowlistRecord> ListAllowlist()
    {
        ...
        using var conn = Connect();
        using var cmd = conn.CreateCommand();
        cmd.CommandText = "SELECT id, target, list_kind, note, enabled, added_by, created_at FROM infrastructure_target_allowlist ORDER BY created_at";
        using var r = cmd.ExecuteReader();
        while (r.Read())
        {
    ...
    // ---- Credentials (rows only — encryption/audit lives in InfrastructureCredentialStore) --------------
    ...
    internal void UpsertCredentialRow(string id, string kind, string targetHost, string secretProtected)
    {
        ...
        lock (_writeLock)
        {
            using var conn = Connect();
            using var cmd = conn.CreateCommand();
            cmd.CommandText = @"INSERT INTO infrastructure_credentials (id, kind, target_host, secret_protected, last_verified, created_at, updated_at)
    ...
    internal string? GetCredentialSecretProtected(string id)
    {
        using var conn = Connect();
        using var cmd = conn.CreateCommand();
        cmd.CommandText = "SELECT secret_protected FROM infrastructure_credentials WHERE id = $id";
        Bind(cmd, "$id", id);
    ...
    internal void SetCredentialVerified(string id)
    {
        lock (_writeLock)
        {
            using var conn = Connect();
            using var cmd = conn.CreateCommand();
            cmd.CommandText = "UPDATE infrastructure_credentials SET last_verified = $now, updated_at = $now WHERE id = $id";
    ...
    internal void DeleteCredentialRow(string id)
    {
        lock (_writeLock)
        {
            using var conn = Connect();
            using var cmd = conn.CreateCommand();
            cmd.CommandText = "DELETE FROM infrastructure_credentials WHERE id = $id";
            Bind(cmd, "$id", id);
            cmd.ExecuteNonQuery();
        }
    }
    ...
    internal IReadOnlyList<CredentialRecord> ListCredentialRows()
    {
        ...
        cmd.CommandText = "SELECT id, kind, target_host, secret_protected IS NOT NULL AND secret_protected != '', last_verified, created_at, updated_at FROM infrastructure_credentials ORDER BY id";
        using var r = cmd.ExecuteReader();
        while (r.Read())
        {
    ...
    public void RecordJobRun(string jobName, bool ok, string message)
    {
        ...
        lock (_writeLock)
        {
            using var conn = Connect();
            using var cmd = conn.CreateCommand();
            cmd.CommandText = @"INSERT INTO infrastructure_meta (key, value, updated_at) VALUES ($key, $value, $now)
    ...
    public (string LastRun, string LastResult)? GetJobState(string jobName)
    {
        using var conn = Connect();
        using var cmd = conn.CreateCommand();
        cmd.CommandText = "SELECT value FROM infrastructure_meta WHERE key = $key";
    ...
    // moved to InfrastructureRepository.Integrations.cs — same public surface, now backed by the generic
