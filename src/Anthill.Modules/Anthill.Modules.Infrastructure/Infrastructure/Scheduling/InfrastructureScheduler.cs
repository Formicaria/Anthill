namespace Anthill.Modules.Infrastructure.Scheduling;

/// <summary>
/// Infrastructure Scheduler skeleton (v1.9.0, NORTH_STAR D4). The single interval background runner that
...
///
/// v1.9.0 scope: skeleton only. Disabled by default (infrastructure_scheduler_enabled config gate), only
...
/// </summary>
public sealed record InfrastructureScheduledJob(
    string Name,
    TimeSpan Interval,
    Func<CancellationToken, System.Threading.Tasks.Task<InfrastructureProviderResult>> Runner);

public sealed class InfrastructureScheduler : IDisposable
{
    private readonly IInfrastructureRepository _repository;
    private readonly SemaphoreSlim _concurrencyGate;
    private readonly List<InfrastructureScheduledJob> _jobs = new();
    ...
    public IReadOnlyList<InfrastructureScheduledJob> Jobs { get { lock (_lock) return _jobs.ToList(); } }

    public InfrastructureScheduler(IInfrastructureRepository repository, int maxConcurrency = 2, double jitterFraction = 0.1)
    {
    ...
    public void Register(InfrastructureScheduledJob job)
    {
        lock (_lock)
        {
            if (_jobs.Any(j => string.Equals(j.Name, job.Name, StringComparison.OrdinalIgnoreCase)))
                throw new InvalidOperationException($"Duplicate infrastructure job name: {job.Name}");
            _jobs.Add(job);
        }
    }
    ...
    public async System.Threading.Tasks.Task<InfrastructureProviderResult> RunOnceAsync(string jobName, CancellationToken ct = default)
    {
        InfrastructureScheduledJob? job;
        lock (_lock) job = _jobs.FirstOrDefault(j => string.Equals(j.Name, jobName, StringComparison.OrdinalIgnoreCase));
        if (job is null) return InfrastructureProviderResult.Failure($"Unknown job: {jobName}");
    ...
    internal TimeSpan NextDelay(InfrastructureScheduledJob job)
    {
        int failures;
    ...
    private async System.Threading.Tasks.Task RunLoop(InfrastructureScheduledJob job, CancellationToken ct)
    {
    ...
    private async System.Threading.Tasks.Task<InfrastructureProviderResult> ExecuteAsync(InfrastructureScheduledJob job, CancellationToken ct)
    {
        await _concurrencyGate.WaitAsync(ct).ConfigureAwait(false);
        try
        {
            InfrastructureProviderResult result;
            try { result = await job.Runner(ct).ConfigureAwait(false); }
            catch (OperationCanceledException) { throw; }
            catch (Exception ex) { result = InfrastructureProviderResult.Failure(ex.Message); }
