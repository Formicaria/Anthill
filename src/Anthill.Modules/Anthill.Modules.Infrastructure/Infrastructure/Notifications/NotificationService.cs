...
namespace Anthill.Modules.Infrastructure.Notifications;

/// <summary>
/// Webhook notifications (v1.11.0, NORTH_STAR Phase 7). Config-gated and DISABLED by default
/// (infrastructure_notifications_enabled). Supports Slack, Discord, and a generic JSON webhook. Fires on
/// health-check failures and incident candidates (pending approvals join in V2.1). Strict timeout,
/// never throws into callers, and every send attempt writes an audit infrastructure_events row that
...
public sealed class NotificationService
{
    ...
    public NotificationService(IInfrastructureRepository repository, TimeSpan? timeout = null)
    {
        _repository = repository;
        _timeout = timeout ?? TimeSpan.FromSeconds(5);
    }

    public static bool Enabled => InfrastructureRuntime.Options.NotificationsEnabled;
    ...
    {
        if (!string.IsNullOrWhiteSpace(InfrastructureRuntime.Options.SlackWebhook))
            yield return ("slack", InfrastructureRuntime.Options.SlackWebhook, "text");
        if (!string.IsNullOrWhiteSpace(InfrastructureRuntime.Options.DiscordWebhook))
            yield return ("discord", InfrastructureRuntime.Options.DiscordWebhook, "content");
        if (!string.IsNullOrWhiteSpace(InfrastructureRuntime.Options.GenericWebhook))
            yield return ("generic", InfrastructureRuntime.Options.GenericWebhook, "");
    }
        ...
        foreach (var (kind, url, key) in Targets())
        {
            var payload = key.Length > 0
                ...
                : JsonSerializer.Serialize(new Dictionary<string, string?>
                {
                    ...
                    ["target"] = alert.Target, ["source"] = "anthill", ["version"] = InfrastructureRuntime.Options.ColonyVersion,
