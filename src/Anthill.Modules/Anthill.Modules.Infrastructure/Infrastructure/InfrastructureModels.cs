...
/// <summary>
/// Infrastructure foundation models (v1.9.0, NORTH_STAR Phase 4). Read-only schemas only: these records
/// describe what exists in the infrastructure, what changed, and what is at risk. Nothing in this file can
...
/// <summary>
/// A stored infrastructure credential. The secret payload is encrypted at rest with the existing
/// FieldCipher and is NEVER exposed through the API — status responses carry only
/// configured/last-verified metadata. Every secret use writes an audit InfrastructureEvent.
/// </summary>
...
/// <summary>
/// One operator-maintained allowlist entry that lets deterministic infrastructure providers reach a
/// trusted private host. This list is consulted ONLY by IInfrastructureTargetGuard — the general SSRF
...
/// </summary>
public sealed class InfrastructureInventoryExport
{
    ...
    [JsonPropertyName("nodes")] public List<InfrastructureNode> Nodes { get; set; } = new();
...
{
    [JsonPropertyName("node_id")] public string NodeId { get; set; } = "";     // infrastructure_nodes.id
