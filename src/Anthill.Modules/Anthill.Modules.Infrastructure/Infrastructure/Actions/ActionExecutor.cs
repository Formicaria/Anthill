...
/// </summary>
public interface IInfrastructureActionRunner
{
    string Name { get; }
...
///  3. the INFRASTRUCTURE_STOP kill switch halts every execution regardless of state,
///  4. every transition lands on the infrastructure_events audit stream (+ change_log),
...
{
    private readonly InfrastructureRepository _repo;
    private readonly IReadOnlyList<IInfrastructureActionRunner> _runners;
    private readonly Func<bool> _isStopped;

    public ActionExecutor(InfrastructureRepository repo, IReadOnlyList<IInfrastructureActionRunner> runners, Func<bool>? isStopped = null)
    {
        _repo = repo;
        _runners = runners;
        _isStopped = isStopped ?? (() => InfrastructureActionControl.IsStopped);
    }
    ...
    public async Task<(bool Ok, string Message)> ExecuteAsync(string id, string executedBy, CancellationToken ct = default)
    {
        ...
        if (_isStopped())
            return (false, "INFRASTRUCTURE_STOP is engaged — no infrastructure action may execute until an operator resumes (POST /infrastructure/actions/resume or delete .anthill/INFRASTRUCTURE_STOP).");
        ...
        {
            SubjectKind = proposal.TargetKind.Length > 0 ? proposal.TargetKind : "infrastructure_action",
    ...
    private static string NoRunnerMessage(ActionProposal p) =>
        ...
        + "actions need the Proxmox write runner: enable infrastructure_proxmox_write_actions_enabled (plus the "
    ...
    {
        _repo.RecordEvent(new InfrastructureEvent
        {
            EventType = eventType, SubjectKind = "infrastructure_action", SubjectId = p.ApprovableId,
...
/// </summary>
public sealed class LocalActionRunner : IInfrastructureActionRunner
{
    private readonly InfrastructureRepository _repo;
    public string Name => "local";

    public LocalActionRunner(InfrastructureRepository repo) => _repo = repo;
...
/// </summary>
public sealed class MockActionRunner : IInfrastructureActionRunner
{
