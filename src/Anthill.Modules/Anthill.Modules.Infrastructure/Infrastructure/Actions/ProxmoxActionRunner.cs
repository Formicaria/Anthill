...
public sealed class ProxmoxActionClient : IDisposable
{
    ...
    public ProxmoxActionClient(string host, int port, IInfrastructureTargetGuard targetGuard,
    ...
    private void EnsureTargetAllowed()
    {
        if (!_targetGuard.IsAllowed(_host))
            throw new InvalidOperationException(
                $"Refused: Proxmox host '{_host}' is not on the infrastructure target allowlist (add it under Infrastructure → Allowlist).");
    }
...
/// the Proxmox integration and the new opt-in gate <c>infrastructure_proxmox_write_actions_enabled</c>
...
/// </summary>
public sealed class ProxmoxActionRunner : IInfrastructureActionRunner
{
