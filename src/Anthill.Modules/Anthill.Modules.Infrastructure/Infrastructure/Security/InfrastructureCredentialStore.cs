...
namespace Anthill.Modules.Infrastructure.Security;

/// <summary>
/// Credential Store abstraction for infrastructure providers (v1.9.0, NORTH_STAR D2). Built on the
/// existing <see cref="IFieldCipher"/> encryption:
...
/// - Every secret use writes an audit InfrastructureEvent (event_type "credential_used").
/// - GetSecret exists for deterministic providers only — never hand a secret to an LLM prompt.
/// </summary>
public sealed class InfrastructureCredentialStore : ICredentialProvider
{
    private readonly InfrastructureRepository _repository;
    private readonly IFieldCipher? _cipher;

    public InfrastructureCredentialStore(InfrastructureRepository repository, IFieldCipher? cipher = null)
    {
        _repository = repository;
        // v3.8.7: null is a supported state. The colony runs unencrypted by default, and a
        // infrastructure that refused to start without a cipher would be stricter than the core.
        _cipher = cipher ?? InfrastructureRuntime.Cipher;
    }
    ...
    public void SaveCredential(string id, string kind, string targetHost, string secret, string savedBy)
    {
        ...
        _repository.UpsertCredentialRow(key, (kind ?? "").Trim(), (targetHost ?? "").Trim(), (_cipher is null ? secret : _cipher.Protect(secret)) ?? "");
    ...
    public string? GetSecret(string id, string usedBy)
    {
        ...
        return stored is null ? null : _cipher is null ? stored : _cipher.Unprotect(stored);
    }
