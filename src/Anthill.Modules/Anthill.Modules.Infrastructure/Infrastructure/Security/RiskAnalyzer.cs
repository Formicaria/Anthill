...
/// <summary>
/// Deterministic risk analysis over the EXISTING infrastructure inventory (v1.13.0, NORTH_STAR Phase 9).
...
/// and routed through the Infrastructure Target Allowlist like every other prober.
/// </summary>
public sealed class RiskAnalyzer
{
    private readonly IInfrastructureRepository _repository;
    private readonly InfrastructureRepository? _credentialSource; // credential statuses live off-interface
    ...
    public RiskAnalyzer(IInfrastructureRepository repository)
    {
        _repository = repository;
        _credentialSource = repository as InfrastructureRepository;
    }
    ...
    public (int Open, int Resolved) Analyze(string analyzedBy = "risk-analysis")
    {
        ...
        if (_credentialSource is not null)
            foreach (var cred in new InfrastructureCredentialStore(_credentialSource).ListStatuses())
    ...
    public System.Threading.Tasks.Task<InfrastructureProviderResult> RunAsync(CancellationToken ct)
    {
        var (open, resolvedCount) = Analyze();
        return System.Threading.Tasks.Task.FromResult(
            InfrastructureProviderResult.Success($"risk analysis ok ({open} open, {resolvedCount} resolved)", open));
    }
}
