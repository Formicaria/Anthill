...
namespace Anthill.Modules.Infrastructure.Approvals;
...
///
/// Everything an operator can approve — today's patch proposals, V2.1's infrastructure action proposals,
...
public interface IApprovable
{
    ...
    /// <summary>"patch" today; "infrastructure_action" (V2.1), "network_change" (V2.4) later.</summary>
    string Kind { get; }
    string Title { get; }
    string Summary { get; }
...
/// infrastructure action (restart service, snapshot VM, run backup...). DELIBERATELY not executable in
...
public sealed class ActionProposal : IApprovable
{
    ...
    [JsonPropertyName("kind")] public string Kind => "infrastructure_action";
    [JsonPropertyName("title")] public string Title { get; set; } = "";
    [JsonPropertyName("summary")] public string Summary { get; set; } = "";
...
/// `ApiHost.Infrastructure`; this module may reference the SDK and nothing else of ours, so the core's
...
public static class ApprovableProjections
{
    ...
    {
        ApprovableId = "infrastructure_action:" + p.ApprovableId,
        Kind = p.Kind,
        Title = p.Title,
        Summary = p.Summary,
