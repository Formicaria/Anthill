...
namespace Anthill.Modules.Infrastructure.Automation;
...
///    the v2.3 approval pipeline, which enforces catalog/approval/INFRASTRUCTURE_STOP/rollback itself,
///  - cooldown per rule + max-runs-per-day + a pending-proposal check give triple loop prevention,
///  - every evaluation that fires lands in automation_runs and the infrastructure_events audit stream.
/// </summary>
...
public sealed class AutomationEngine
{
    ...
    public AutomationEngine(InfrastructureRepository repo, ActionExecutor? actions = null,
