...
namespace Anthill.Modules.Infrastructure.Integrations;

/// <summary>
/// Secret-free, network-free status of one infrastructure provider (v1.9.1, NORTH_STAR Phase 5).
...
/// </summary>
public sealed class InfrastructureProviderStatus
{
...
/// infrastructure provider follows from v1.10+ (NORTH_STAR Phase 5 "one shared pattern"):
///
...
///   <see cref="IInfrastructureTargetGuard"/> first and fails without "contacting" anything when the
///   host is not allowlisted — the exact D1 wiring real providers must copy.
/// - Every run records a infrastructure event; status is available secret-free at any time.
/// - Thread-safe so the scheduler's concurrency cap can be exercised in tests.
/// </summary>
public abstract class FakeInfrastructureProvider : IIntegrationStatusProvider
{
    private readonly object _lock = new();
    private readonly IInfrastructureRepository _repository;
    private readonly IInfrastructureTargetGuard? _targetGuard;
    ...
    protected FakeInfrastructureProvider(string name, string kind, int itemCount, IInfrastructureRepository repository,
        IInfrastructureTargetGuard? targetGuard = null, string targetHost = "")
    {
    ...
    public async System.Threading.Tasks.Task<InfrastructureProviderResult> RunAsync(CancellationToken ct)
    {
        if (!Enabled) return Record(InfrastructureProviderResult.Failure("provider disabled"));
        ...
        if (TargetHost.Length > 0 && _targetGuard is not null && !_targetGuard.IsAllowed(TargetHost))
            return Record(InfrastructureProviderResult.Failure($"target '{TargetHost}' is not on the infrastructure allowlist"));
        ...
        return Record(injectedFailure
            ? InfrastructureProviderResult.Failure("injected failure (mock)")
            : InfrastructureProviderResult.Success($"{Kind} sync ok (mock)", ItemCount));
    }

    private InfrastructureProviderResult Record(InfrastructureProviderResult result)
    {
        lock (_lock)
        {
            _runs++;
        ...
        _repository.RecordEvent(new InfrastructureEvent
        {
    ...
    public InfrastructureProviderStatus Status()
    {
        lock (_lock) return new InfrastructureProviderStatus
        {
...
public sealed class FakeProxmoxProvider : FakeInfrastructureProvider, IInventoryProvider
{
    public FakeProxmoxProvider(IInfrastructureRepository repository, IInfrastructureTargetGuard? guard = null, string targetHost = "")
        : base("fake-proxmox", "proxmox", itemCount: 7, repository, guard, targetHost) { }
    public System.Threading.Tasks.Task<InfrastructureProviderResult> SyncInventoryAsync(CancellationToken ct) => RunAsync(ct);
}

/// <summary>Mock DNS provider: pretends to read a zone. Shape shared with the V2.4 IDnsProvider.</summary>
public sealed class FakeDnsProvider : FakeInfrastructureProvider
{
    public FakeDnsProvider(IInfrastructureRepository repository, IInfrastructureTargetGuard? guard = null, string targetHost = "")
        : base("fake-dns", "dns", itemCount: 12, repository, guard, targetHost) { }
}

/// <summary>Mock DHCP provider: pretends to read leases.</summary>
public sealed class FakeDhcpProvider : FakeInfrastructureProvider
{
    public FakeDhcpProvider(IInfrastructureRepository repository, IInfrastructureTargetGuard? guard = null, string targetHost = "")
...
public sealed class FakeFirewallProvider : FakeInfrastructureProvider
{
    public FakeFirewallProvider(IInfrastructureRepository repository, IInfrastructureTargetGuard? guard = null, string targetHost = "")
...
public sealed class FakeHealthProvider : FakeInfrastructureProvider, IHealthCheckProvider
{
    public FakeHealthProvider(IInfrastructureRepository repository, IInfrastructureTargetGuard? guard = null, string targetHost = "")
