...
namespace Anthill.Modules.Infrastructure.Integrations.Hyperv;
...
public sealed class HypervWinRmClient
{
    ...
    public HypervWinRmClient(string host, int port, IInfrastructureTargetGuard targetGuard,
    ...
    internal HypervWinRmClient(string endpoint, IInfrastructureTargetGuard targetGuard, Func<string?> credentialProvider, TimeSpan? timeout = null)
    {
        Endpoint = endpoint;
    ...
    {
        if (Host.Length == 0) throw new InvalidOperationException("Hyper-V host is not configured (infrastructure_hyperv_host).");
        if (!_targetGuard.IsAllowed(Host))
            throw new InvalidOperationException($"Hyper-V host '{Host}' is not on the infrastructure target allowlist — add it under /infrastructure/allowlist.");
        var cred = _credentialProvider();
        if (string.IsNullOrWhiteSpace(cred) || !cred.Contains(':'))
            throw new InvalidOperationException("Hyper-V credential is not configured — save 'user:password' under /infrastructure/credentials and set infrastructure_hyperv_credential_id.");
...
public sealed class HypervInventoryProvider : IInventoryProvider, IIntegrationStatusProvider
{
    ...
    public HypervInventoryProvider(HypervWinRmClient client, IInfrastructureRepository repository)
    {
        _client = client;
    ...
    public async System.Threading.Tasks.Task<InfrastructureProviderResult> SyncInventoryAsync(CancellationToken ct)
    {
        try
        {
            ...
            return InfrastructureProviderResult.Success($"hyperv sync ok ({items} items)", items);
        }
