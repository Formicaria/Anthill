...
namespace Anthill.Modules.Infrastructure.Integrations.VSphere;
...
/// every infrastructure provider: the host must pass the target allowlist before any request; the credential
...
///
/// Enterprise transport: HTTPS to vCenter/ESXi (default 443). Self-signed certs → infrastructure_esxi_insecure_tls.
...
public sealed class VSphereApiClient
{
    ...
    public VSphereApiClient(string host, int port, IInfrastructureTargetGuard targetGuard,
    ...
    internal VSphereApiClient(string baseUrl, IInfrastructureTargetGuard targetGuard, Func<string?> credentialProvider, TimeSpan? timeout = null)
    {
    ...
    {
        if (Host.Length == 0) throw new InvalidOperationException("vSphere/ESXi host is not configured (infrastructure_esxi_host).");
        if (!_targetGuard.IsAllowed(Host))
            throw new InvalidOperationException($"vSphere host '{Host}' is not on the infrastructure target allowlist — add it under /infrastructure/allowlist.");
    }
    ...
    private async System.Threading.Tasks.Task EnsureSessionAsync(CancellationToken ct)
    {
        ...
        if (string.IsNullOrWhiteSpace(cred) || !cred.Contains(':'))
            throw new InvalidOperationException("vSphere credential is not configured — save 'username:password' under /infrastructure/credentials and set infrastructure_esxi_credential_id.");
        ...
        using var resp = await InfrastructureHttpClients.Pick(_insecureTls).SendAsync(req, HttpCompletionOption.ResponseHeadersRead, cts.Token).ConfigureAwait(false);
...
public sealed class VSphereInventoryProvider : IInventoryProvider, IIntegrationStatusProvider
{
    ...
    public VSphereInventoryProvider(VSphereApiClient client, IInfrastructureRepository repository)
    {
        _client = client;
    ...
    public async System.Threading.Tasks.Task<InfrastructureProviderResult> SyncInventoryAsync(CancellationToken ct)
    {
        try
        {
            var items = 0;
            ...
            foreach (var host in Arr(await _client.GetHostsAsync(ct).ConfigureAwait(false)))
            {
                ...
                _repository.UpsertNode(new InfrastructureNode
                {
            ...
            return InfrastructureProviderResult.Success($"esxi sync ok ({items} items)", items);
        }
        ...
        InfrastructureProviderResult Fail(string message)
        {
            ...
            return InfrastructureProviderResult.Failure(message);
        }
    }
