...
namespace Anthill.Modules.Infrastructure.Integrations.Arr;
...
/// on the D1 infrastructure allowlist, strict timeout, secrets never logged.
/// </summary>
public sealed class ArrClient : IDisposable
{
    ...
    public ArrClient(string url, IInfrastructureTargetGuard targetGuard, Func<string?> apiKeyProvider)
    {
    ...
    public async Task<JsonElement> GetAsync(string relativePath, CancellationToken ct)
    {
        if (!_targetGuard.IsAllowed(_base.Host))
            throw new InvalidOperationException($"Refused: '{_base.Host}' is not on the infrastructure target allowlist.");
...
/// integration never fails the sweep. The job name stays "arr-sync" for infrastructure_meta continuity.
/// </summary>
public sealed class IntegrationSyncProvider
{
    ...
    public IntegrationSyncProvider(InfrastructureRepository repository, IInfrastructureTargetGuard targetGuard, Func<string, string?> credentialLookup)
    {
    ...
    public async Task<InfrastructureProviderResult> RunAsync(CancellationToken ct)
    {
        ...
        return new InfrastructureProviderResult(failed == 0, $"integration sync: {ok} ok, {failed} failed of {instances.Count}");
    }
