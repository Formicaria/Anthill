...
namespace Anthill.Modules.Infrastructure.Integrations.Docker;
...
/// are STRUCTURALLY impossible, exactly like the Proxmox client. Discipline mirrors every infrastructure
...
public sealed class DockerApiClient
{
    ...
    public DockerApiClient(string host, int port, IInfrastructureTargetGuard targetGuard,
    ...
    internal DockerApiClient(string baseUrl, IInfrastructureTargetGuard targetGuard, Func<string?> tokenProvider, TimeSpan? timeout = null)
    {
    ...
    {
        if (Host.Length == 0) throw new InvalidOperationException("Docker host is not configured (infrastructure_docker_host).");
        if (!_targetGuard.IsAllowed(Host))
            throw new InvalidOperationException($"Docker host '{Host}' is not on the infrastructure target allowlist — add it under /infrastructure/allowlist.");
...
/// its containers and volumes into the shared infrastructure inventory (stable ids → re-sync is idempotent).
...
public sealed class DockerInventoryProvider : IInventoryProvider, IIntegrationStatusProvider
{
    ...
    public DockerInventoryProvider(DockerApiClient client, IInfrastructureRepository repository)
    {
        _client = client;
    ...
    public async System.Threading.Tasks.Task<InfrastructureProviderResult> SyncInventoryAsync(CancellationToken ct)
    {
        try
        {
            ...
            return InfrastructureProviderResult.Success($"docker sync ok ({items} items)", items);
        }
