...
namespace Anthill.Modules.Infrastructure.Integrations.Proxmox;
...
///
/// Discipline (same as every infrastructure provider):
/// - The target host must pass the Infrastructure Target Allowlist (D1) before ANY request.
...
{
    // TLS trust: infrastructure Proxmox nodes almost always run self-signed certs. Verification is
    // config-controlled (infrastructure_proxmox_insecure_tls, default false = verify). Two shared
    ...
    public ProxmoxApiClient(string host, int port, IInfrastructureTargetGuard targetGuard,
    ...
    internal ProxmoxApiClient(string baseUrl, IInfrastructureTargetGuard targetGuard, Func<string?> tokenProvider, TimeSpan? timeout = null)
    {
    ...
    {
        if (Host.Length == 0) throw new InvalidOperationException("Proxmox host is not configured (infrastructure_proxmox_host).");
        if (!_targetGuard.IsAllowed(Host))
            throw new InvalidOperationException($"Proxmox host '{Host}' is not on the infrastructure target allowlist — add it under /infrastructure/allowlist.");
        var token = _tokenProvider();
        if (string.IsNullOrWhiteSpace(token))
            throw new InvalidOperationException("Proxmox API token credential is not configured (save it under /infrastructure/credentials and set infrastructure_proxmox_credential_id).");
