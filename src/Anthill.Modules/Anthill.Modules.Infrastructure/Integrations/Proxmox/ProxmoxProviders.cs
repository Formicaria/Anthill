...
/// <summary>One failed Proxmox task worth surfacing (v1.12.0). Recorded as a infrastructure event.</summary>
...
/// upserts them into the infrastructure inventory (stable ids — re-sync is idempotent). Deterministic C#;
...
public sealed class ProxmoxInventoryProvider : IInventoryProvider, IIntegrationStatusProvider
{
    ...
    public ProxmoxInventoryProvider(ProxmoxApiClient client, IInfrastructureRepository repository)
    {
        _client = client;
    ...
    public async System.Threading.Tasks.Task<InfrastructureProviderResult> SyncInventoryAsync(CancellationToken ct)
    {
        try
        {
            ...
            return InfrastructureProviderResult.Success($"proxmox sync ok ({items} items)", items);
        }
