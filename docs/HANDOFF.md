...
Shipping release: **v0.3.9.10**. Everything below the rule is a record, not a state.
...
plain question as fourteen tasks with a coder. `.147` came from a session of real chat traffic on that colony after `.146` landed: the class worked (three questions, three `class_needs_no_plan` plans, no coder), and everything AROUND the answer was wrong — all three good answers graded `partial` because their verifiers said `Needs Improvement` and `.140` counted that as a verdict of no; `[d1]` was an internal deliverable id printed above the operator's own question on a one-part answer; and a plan of `researcher + verifier` had nobody to compile an answer at all, which `EnforceConstraints` now guarantees the way it has always guaranteed the verifier. `.148` answered the question a fresh install could not: asked what MICROMOUND is, a real colony searched mission memory, found tacos, and reported it had no record of the term — so ANTHILL now ships eleven entries describing itself, read through `colony_self_knowledge`, recording no evidence (on `system_info`'s precedent, so an audit cannot be satisfied by a shipped paragraph) and dispatched unconditionally rather than on a keyword trigger. It also made project→knowledge-base binding an API operation instead of a hand-edited config key, which the FORAGER contract §3 already required, and added P11/P12 to that contract for the project LISTING rather than inventing `GET /api/projects` consumer-side. `.149`-`.152` are in `CHANGELOG.md`. `.153` found two knowledge routes that had shipped with no caller anywhere in the console. `.154` made a bound knowledge base something the colony can be told to STUDY -- one mission per unstudied document, up to 25 a click, on receipts that survive a restart. `.155` gave `knowledge_review` the lifecycle it had lacked for thirty-four releases: a role that can raise one, an operator surface that answers it, and no claim that accepting applies anything, because FORAGER publishes no way to apply a review (P13). `.156` put the study pass on a schedule (`knowledge_auto_study`, off by default) and guaranteed a PLAN STEP that reads a bound knowledge base before anything writes the answer. `.157` found that step reached nobody -- the researcher is a deterministic handler and never dispatched the tool the step's description asked for -- made what it retrieves citable as `knowledge:<project>/<id>` so a knowledge-backed claim stops rendering `[UNSOURCED]`, rebuilt the Knowledge page around connect -> bind -> import with everything else folded, and made the study schedule and the endpoint console-writable, the endpoint under a loopback-only value guard. `.158`-`.160` are in `CHANGELOG.md`. `.9` opened the 3.9 line with the memory vault — an Obsidian-shaped sidebar over every record the colony keeps, seated as dots in the chamber the server assigns; `.9.1` fixed its panel and dot defects; `.9.2` closed the review lifecycle against FORAGER 0.6's apply route and made the derived same-subject edge real rather than described. `.9.3` is A4: the colony now tells a document it has never studied from one whose content has MOVED since it did, files that as a FINDING without acting on it, and lets the operator decide — `knowledge_auto_study` gained a third mode, `suggest`, which runs the identical pass with queueing off. It also moved 6,600 lines of pre-renumbering history out of a 1.2 MB `CHANGELOG.md` into `docs/archive/CHANGELOG-pre-v0.3.md`, unedited, with a guard on the seam. `.9.4` is the vault made usable, from five reports in one sitting: the topology poll had been erasing its dots on a timer (`vaultChambers` was set and never read), the 96-slot lattice pinned every record past the 96th to one radius on one of 96 directions, a stratum was a flat plate whatever it held, the record card was a SIBLING of the panel the one click listener was bound to so its ✕ and its links were all inert, and the panel followed the toolbar button rather than the focused chamber. `.9.5` is one line: the Knowledge page's **Bind** button UNBOUND. `ReadFromJsonAsync` uses camelCase with case-insensitive matching, which does not bridge `knowledge_base`, so the field arrived null and an empty knowledge base legitimately means unbind. `RequestWireNameTests` now scans every deserialized request record in the API for the next one — the rule had been written in prose in `ApiHost.Micromound.cs` since `.114` and the Knowledge module was written without reading it. `.9.6` is the other half of the same button: the write reached the FILE and never re-projected, so `Knowledge.ProjectMap` — the copy `Queen.ResolveKnowledgeScope` reads — kept the old map until restart. Bind succeeded on disk while the colony went on retrieving nothing. `SetKnowledgeBinding` does both halves under one lock, which is verbatim what `.96` did for model routes after the identical mistake. `.9.7` answered "is the live view slow because there are too many particles" with no: each grain cost four redundant trig calls (`proj` recomputed the camera basis PER POINT, while `lightPrep` had cached it per frame all along), a CSS colour string the canvas had to parse, and its own fill. Batched by quantised colour, with a stable index-stride sample when a chamber is not focused. `.9.8` came from the first Study pass that ever completed, read out of the live colony database: all fifty seeded missions planned `builder -> verifier`, never opened the knowledge base, and graded `completed_verified` at 1.0 while saying they had found nothing. The seeder phrases its goal as a question (for the `Observe` ceiling), a question is `simple_answer`, and `simple_answer` is the one class the knowledge step is excluded from — so the lane built to read a bound base was the lane that could never read it. Intake no longer puts a question ABOUT the knowledge base in that class. `.9.9` found that fix inert: `Queen.RunMission` enters the knowledge scope 130 lines BELOW `MissionContext.Create`, and the context is where classification happens, so `HasScope` was always false at the one moment intake asked. The entry moved above it. `.9.8`'s own test had entered the scope by hand before calling `Resolve` - a test of a different program. `PolicyVersion` went to `v2` so the 1,602 documents already holding useless receipts can be studied again.
