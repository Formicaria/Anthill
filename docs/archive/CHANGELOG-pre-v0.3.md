# CHANGELOG - the frozen history, v1.0 through v3.8.33

**THIS FILE IS A RECORD, NOT A DOCUMENT UNDER MAINTENANCE.** Everything below shipped before the
line was renumbered from `3.8.x` to `0.3.8.x` at v0.3.8.34. It was moved here from `CHANGELOG.md` at
v0.3.9.3, unedited - not one heading, link or sentence was changed on the way, because a shipped
entry is a record of its own moment and correcting one in place is the edit this repository's own
rules forbid.

WHY IT MOVED. `CHANGELOG.md` had reached 1.2 MB and 16,455 lines, of which 6,600 were history nobody
reads and no guard polices: `DocsConsistencyTests` scopes its uniqueness and ordering checks to the
active line precisely because these headings contain fifteen duplicates and several out-of-order
entries that are frozen and will stay that way. Carrying them in the file an operator opens to answer
"what is in this release" cost every reader - and every test that reads the changelog - 40% of the
file for nothing.

It lives under `docs/archive/` for one concrete reason as well as the obvious one:
`EveryDocumentationLink_PointsAtAFileThatExists` excludes that directory, because an archived
document's links describe the world as it was. The same argument that exempted `docs/archive/v2/`
exempts this.

The active changelog is `CHANGELOG.md`, at the repository root.
