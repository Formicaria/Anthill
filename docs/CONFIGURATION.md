...
| `knowledge_forager_token` | string | _(secret)_ | yes | `ANTHILL_KNOWLEDGE_FORAGER_TOKEN` | **required when `knowledge_enabled`** — FORAGER authenticates every `/api` route; an `fgr_` integration token minted by the FORAGER operator. Empty means 401 on every call |
