...
| P13 | **Apply a review decision to a knowledge item.** | §8 | **DELIVERED by the producer, verified at v0.3.9.2 against FORAGER 0.6.2's routes**: `POST /api/knowledge/:id/review` with `{action, notes?, actor?}`, scope `review`, taking exactly the four actions this colony proposes — `mark_reviewed`, `reject`, `restore`, `archive`. No mapping was needed and none was invented. ANTHILL consumes it through `IKnowledgeIngestionProvider.ApplyReviewAsync`; the proposal lifecycle gains the `applied` status `.155` deliberately refused to declare while the endpoint did not exist |
