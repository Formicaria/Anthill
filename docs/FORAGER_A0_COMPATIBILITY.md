...
| Pinned item | Value |
|---|---|
| Producer version window | `>= 0.1.4 && schema_version == 1 && anthill package_version in [1..5]`. **Re-pinned 2026-09-11 (W3-04):** the ceiling read `== 1` while `src/server/adapters/exports/anthill.ts:24` has shipped `ANTHILL_PACKAGE_VERSION = 5` since v0.7.0, so the stated window rejected every package the current producer writes. Versions 2–5 are additive over 1 (2 adds `subjects.jsonl`; 3 re-attributes `statements` vs `mentioned_in`; 4 adds `identity`/`facts`/`as_written`/`source_names`; 5 drops subject rows that carry nothing, counting them as `subjects_named_only`), so a v1 consumer still reads a v5 package. Version read from `GET /api/ready` (`version`, `schema_version`); package version validated from `anthill-package.json` (`package_version`) on every package consumed |
...
| Auth posture | **CORRECTED 2026-09-11 (W3-04). FORAGER authenticates every `/api` route** and has since its 0.7.0 (`src/server/app.ts:44`, `app.use('/api', authenticate(...))` mounted ahead of every router). Public routes are `/health`, `/ready`, `/openapi.json` and `/session` — nothing else. `knowledge_forager_token` is therefore **required, not optional**: a colony without one gets 401 on every retrieval and ingestion call. The credential is an `fgr_` integration token the FORAGER operator mints (`services/access.ts`), carrying scopes (`read, ingest, review, export, projects`), an optional project limit, an expiry and immediate revocation. Verified live against Forager 0.7.0: `GET /api/ready` → 200, `GET /api/capabilities` with no header → **401** (not 404 — the middleware precedes the router). The previous text — "no authentication of any kind" — was false from FORAGER's 2026-09-08 `007_operator_access` onward |
