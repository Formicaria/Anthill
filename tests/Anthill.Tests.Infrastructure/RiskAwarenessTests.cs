...
public class RiskAwarenessTests : IDisposable
{
    ...
    public RiskAwarenessTests()
    {
        ...
        _repo = new InfrastructureRepository(Path.Combine(_dir, "risk.db"));
    ...
    {
        _repo.UpsertNode(new InfrastructureNode { Name = "nas", Address = "192.168.1.10" }, "t");
        _repo.UpsertNode(new InfrastructureNode { Name = "pve", Address = "192.168.1.11" }, "t");
    ...
    public void UnknownDevice_OwnerlessService_MissingDns_UnwatchedService_UnverifiedCredential()
    {
        ...
        _repo.UpsertNode(new InfrastructureNode { Name = "nas", Address = "192.168.1.10" }, "t"); // bare IP, no dot in name
        _repo.UpsertService(new ServiceRecord { Name = "jellyfin", Owner = "", Ports = { 8096 } }, "t"); // no owner, no check
        var store = new InfrastructureCredentialStore(_repo);
    ...
    {
        var host = new InfrastructureNode { Id = "h1", Name = "pve1", Address = "10.0.0.2" };
    ...
    public void Devices_RoundTripThroughInventoryExport()
    {
        ...
        using var target = new InfrastructureRepository(Path.Combine(_dir, "target.db"));
