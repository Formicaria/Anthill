...
/// </summary>
[Collection("InfrastructureRuntimeConfig")] // serialize: these tests mutate AnthillRuntime notification statics
public class HealthAndNotificationTests : IDisposable
{
    private readonly string _dir;
    private readonly InfrastructureRepository _repo;
    private readonly InfrastructureTargetGuard _guard;
    private readonly InfrastructureOptions _savedOptions;

    public HealthAndNotificationTests()
    {
        _dir = Path.Combine(Path.GetTempPath(), "anthill_health_" + Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(_dir);
        _repo = new InfrastructureRepository(Path.Combine(_dir, "health.db"));
        _guard = new InfrastructureTargetGuard(_repo);
        // v3.8.7: notification settings live on the module now, not the core runtime — the
        // infrastructure reads InfrastructureRuntime.Options, which a composition root supplies.
        _savedOptions = InfrastructureRuntime.Options;
    }

    public void Dispose()
    {
        InfrastructureRuntime.Configure(_savedOptions);
        _repo.Dispose();
    ...
    private static void ConfigureNotifications(bool enabled, string? generic) =>
        InfrastructureRuntime.Configure(InfrastructureRuntime.Options with
        {
