...
public class InventoryRegistryTests : IDisposable
{
    ...
    private static (InfrastructureNode Node, ServiceRecord Service, DependencyRecord Dep) Seed(InfrastructureRepository repo)
    {
        var node = new InfrastructureNode { Name = "pve1", Kind = "hypervisor", Address = "192.168.1.5", RoleTags = { "hypervisor" } };
    ...
    {
        using var source = new InfrastructureRepository(NewDbPath());
        ...
        using var target = new InfrastructureRepository(NewDbPath());
    ...
    {
        using var repo = new InfrastructureRepository(NewDbPath());
        var bundle = new InfrastructureInventoryExport
        {
            Nodes = { new InfrastructureNode { Name = "" } },                                  // nameless — skipped
    ...
    public void Export_NeverContainsCredentialOrAllowlistMaterial()
    {
        using var repo = new InfrastructureRepository(NewDbPath());
        Seed(repo);
        var store = new Anthill.Modules.Infrastructure.Security.InfrastructureCredentialStore(repo);
