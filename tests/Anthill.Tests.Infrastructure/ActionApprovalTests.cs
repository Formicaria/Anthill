...
/// injected as a Func&lt;bool&gt; so tests never touch the real .anthill/INFRASTRUCTURE_STOP sentinel.
/// </summary>
public class ActionApprovalTests : IDisposable
{
    ...
    private static (ActionExecutor Executor, InfrastructureRepository Repo, MockActionRunner Mock) Harness(
        string dbPath, Func<bool>? stopped = null)
    {
        var repo = new InfrastructureRepository(dbPath);
        var mock = new MockActionRunner();
        var executor = new ActionExecutor(repo, new IInfrastructureActionRunner[] { new LocalActionRunner(repo), mock }, stopped ?? (() => false));
    ...
    public async Task Execute_RefusesWhileKillSwitchEngaged()
    {
        ...
        var (ok, message) = await executor.ExecuteAsync(proposal.ApprovableId, "tester");
        Assert.False(ok);
        Assert.Contains("INFRASTRUCTURE_STOP", message);
        Assert.Empty(mock.Executed);
    ...
    {
        // Scoped correctly (NORTH_STAR gate 10): the infrastructure sentinel name is its own file,
        // distinct from the autonomy STOP sentinel.
        Assert.NotEqual(AnthillRuntime.AutonomyStopFileName, AnthillRuntime.InfrastructureStopFileName);
    }
    ...
    public void UnifiedProjection_CarriesActionProposalIntoTheOneQueue()
    {
        ...
        Assert.Equal("infrastructure_action", view.Kind);
        ...
        Assert.StartsWith("infrastructure_action:", view.ApprovableId);
    }
    ...
    {
        Assert.False(AnthillRuntime.ApiPermissions.GetValueOrDefault("approve_infrastructure_actions", true));
        Assert.False(AnthillRuntime.ApiPermissions.GetValueOrDefault("execute_infrastructure_actions", true));
    }
}
