...
public class IncidentMemoryTests : IDisposable
{
    ...
    public IncidentMemoryTests()
    {
        ...
        _repo = new InfrastructureRepository(Path.Combine(_dir, "inc.db"));
    ...
    {
        _repo.RecordEvent(new InfrastructureEvent { EventType = "incident_candidate", SubjectKind = "health_check", SubjectId = "svc-a:80", Severity = "error", Message = "3 consecutive failures" });
        _repo.RecordEvent(new InfrastructureEvent { EventType = "incident_candidate", SubjectKind = "health_check", SubjectId = "svc-b:443", Severity = "error", Message = "3 consecutive failures" });
    ...
    public void Approvable_ActionProposal_IsInertAndFailsTowardCaution()
    {
        ...
        Assert.Equal("infrastructure_action", proposal.Kind);
