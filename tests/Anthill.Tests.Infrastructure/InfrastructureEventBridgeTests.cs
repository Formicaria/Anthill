...
///
/// <c>RecordEvent</c> has been the infrastructure's own event stream since v1.9.0: its own table, its own
...
/// </summary>
public class InfrastructureEventBridgeTests : IDisposable
{
    private readonly string _dir;

    public InfrastructureEventBridgeTests()
    {
    ...
    private InfrastructureRepository NewRepo() =>
        new(Path.Combine(_dir, Guid.NewGuid().ToString("N") + ".db"));

    private static readonly TimeSpan Timeout = TimeSpan.FromSeconds(5);

    private static InfrastructureEvent Event(string id = "e1", string type = "inventory_changed") => new()
    {
        Id = id,
        EventType = type,
        SubjectKind = "vm",
    ...
    /// property existed. Every infrastructure test constructs one without a bus.
    /// </summary>
    [Fact]
    ...
    public async Task A_recorded_event_reaches_the_colony_stream()
    {
        ...
        // Prefixed, not passed through: infrastructure draws from a different type vocabulary, and a
        ...
        Assert.Equal("infrastructure_inventory_changed", published.EventType);
        Assert.Equal("inventory drifted", published.Message);
        Assert.Equal("inventory_changed", published.Metadata["infrastructure_event_type"]);
    ...
    /// <summary>
    /// The wrinkle the mission log does not have. Infrastructure inserts are OR IGNORE because providers
