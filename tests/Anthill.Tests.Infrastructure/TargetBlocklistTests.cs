...
public class TargetBlocklistTests : IDisposable
{
    ...
    public void Migration_ListKindColumn_AddedToPre254Databases_DefaultingAllow()
    {
        ...
        using (var conn = new SqliteConnection($"Data Source={dbPath}"))
        {
            ...
            cmd.CommandText = @"CREATE TABLE infrastructure_target_allowlist (
                ...
                INSERT INTO infrastructure_target_allowlist (id,target,note,enabled,added_by,created_at)
                VALUES ('legacy','nas.lan','old row',1,'tester','2026-01-01T00:00:00Z')";
            cmd.ExecuteNonQuery();
        }
        SqliteConnection.ClearAllPools();

        using var repo = new InfrastructureRepository(dbPath); // InitDb adds list_kind idempotently
        ...
        Assert.True(new InfrastructureTargetGuard(repo).IsAllowed("nas.lan"));

        // Re-open twice more: the ALTER guard must be idempotent.
        repo.Dispose();
        using (var again = new InfrastructureRepository(dbPath)) Assert.Single(again.ListAllowlist());
    }
}
