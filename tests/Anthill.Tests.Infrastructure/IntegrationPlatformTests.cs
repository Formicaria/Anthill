...
public class IntegrationPlatformTests : IDisposable
{
    ...
    private sealed class FakeGuard : IInfrastructureTargetGuard
    {
    ...
    public void LegacyArrAppsRows_MigrateToIntegrationTables_OnOpen()
    {
        ...
        using (var repo = new InfrastructureRepository(dbPath)) { } // create schema (arr_apps still exists, empty)
        ...
        using (var repo = new InfrastructureRepository(dbPath)) // reopen → InitDb migrates
        {
            var app = Assert.Single(repo.ListArrApps());
        ...
        using (var repo = new InfrastructureRepository(dbPath)) // third open is a no-op
