...
/// prevention; INFRASTRUCTURE_STOP). Fixed injected clock — nothing can flake.
/// </summary>
public class AutomationRuleTests : IDisposable
{
    ...
    private InfrastructureRepository Repo()
    {
        ...
        return new InfrastructureRepository(Path.Combine(_dir, Guid.NewGuid().ToString("N") + ".db"));
    }
    ...
    private void FillDisk(InfrastructureRepository repo, int pct)
    ...
    {
        var repo = Repo();
        var executor = new ActionExecutor(repo, new List<IInfrastructureActionRunner> { new MockActionRunner() }, isStopped: () => false);
