...
/// and the infrastructure executor — the only production system that changes external state — never
...
public class ActionLifecycleBridgeTests : IDisposable
{
    ...
    private static (ActionExecutor Executor, InfrastructureRepository Repo, MockActionRunner Mock) Harness(string dbPath)
    {
        var repo = new InfrastructureRepository(dbPath);
        var mock = new MockActionRunner();
        var executor = new ActionExecutor(repo, new IInfrastructureActionRunner[] { mock }, () => false);
    ...
    private sealed class VerifyFailsRunner : IInfrastructureActionRunner
    {
    ...
    {
        var repo = new InfrastructureRepository(NewDbPath());
        var executor = new ActionExecutor(repo, new IInfrastructureActionRunner[] { new VerifyFailsRunner() }, () => false);
        var (proposal, _) = executor.Propose(Request(), "tester");
        executor.Approve(proposal!.ApprovableId, "approver");
    ...
    public async Task ARunnerFailure_RecordsTheAttemptAsFailed_ButRetryStaysExplicit()
    {
        ...
        var executor = new ActionExecutor(repo, new IInfrastructureActionRunner[] { failing }, () => false);
        var (proposal, _) = executor.Propose(Request(), "tester");
        executor.Approve(proposal!.ApprovableId, "approver");
    ...
    private sealed class FailingRunner : IInfrastructureActionRunner
    {
        public bool Succeed;
