...
public class MockProviderHarnessTests : IDisposable
{
    ...
    public MockProviderHarnessTests()
    {
        ...
        _repo = new InfrastructureRepository(Path.Combine(_dir, "mock.db"));
    }
    ...
    private FakeInfrastructureProvider Create(string kind, IInfrastructureTargetGuard? guard = null, string host = "") => kind switch
    {
    ...
    {
        var guard = new InfrastructureTargetGuard(_repo);
    ...
    {
        using var scheduler = new InfrastructureScheduler(_repo, maxConcurrency: 2);
        var providers = new[] { "proxmox", "dns", "dhcp", "firewall", "health" }.Select(k => Create(k)).ToList();
        foreach (var p in providers)
            scheduler.Register(new InfrastructureScheduledJob(p.Name, TimeSpan.FromMinutes(5), p.RunAsync));
    ...
    {
        using var scheduler = new InfrastructureScheduler(_repo, maxConcurrency: 2, jitterFraction: 0.0);
        var provider = Create("dns");
        var job = new InfrastructureScheduledJob(provider.Name, TimeSpan.FromMinutes(10), provider.RunAsync);
    ...
    public async System.Threading.Tasks.Task Scheduler_ConcurrencyCapIsRespected_NoCheckStampede()
    {
        ...
        {
            var name = $"slow-job-{i}";
            scheduler.Register(new InfrastructureScheduledJob(name, TimeSpan.FromMinutes(5), async ct =>
            {
                ...
                return InfrastructureProviderResult.Success("slow ok");
            }));
        }
    ...
    public async System.Threading.Tasks.Task Scheduler_StartRunsJobsInBackground_AndStopHalts()
    {
        using var scheduler = new InfrastructureScheduler(_repo, maxConcurrency: 2);
        var runs = 0;
        scheduler.Register(new InfrastructureScheduledJob("ticker", TimeSpan.FromMilliseconds(120), _ =>
        {
            Interlocked.Increment(ref runs);
            return System.Threading.Tasks.Task.FromResult(InfrastructureProviderResult.Success("tick"));
        }));
