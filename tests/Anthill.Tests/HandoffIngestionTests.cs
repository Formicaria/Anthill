...
public class HandoffIngestionTests : IDisposable
{
    ...
    public void AnAdmittedHandoff_BecomesARealScheduledAndPersistedTask()
    {
        ...
        {
            queen.IngestHandoffs(mission, Context(mission), source, execution, Selection(source), scheduler);
            return 0;
        }));
    ...
    public void WithTheFeatureGateOff_NothingIsAdmitted()
    {
        ...
        WithMedicOpen(() =>
        {
            queen.IngestHandoffs(mission, Context(mission), source, ResultProposing(Medic("k-off")), Selection(source), scheduler);
            return 0;
        });
    ...
    public void AHandoffToAGatedRole_IsRefused_AndTheRefusalIsRecorded()
    {
        ...
        WithIngestion(() =>
        {
            queen.IngestHandoffs(mission, Context(mission), source, ResultProposing(Medic("k-gated")), Selection(source), scheduler);
            return 0;
        });
    ...
    public void TheSameHandoffTwice_AdmitsOnce()
    {
        ...
        WithIngestion(() => WithMedicOpen(() =>
        {
            ...
            queen.IngestHandoffs(mission, context, source, ResultProposing(Medic("k-dupe")), Selection(source), scheduler);
            queen.IngestHandoffs(mission, context, source, ResultProposing(Medic("k-dupe")), Selection(source), scheduler);
            return 0;
        }));
    ...
    public void AHandoffChain_TerminatesAtTheDepthLimit()
    {
        ...
        WithIngestion(() => WithMedicOpen(() =>
        {
            ...
            {
                queen.IngestHandoffs(mission, context, current, ResultProposing(Medic($"k-chain-{i}")), Selection(current), scheduler);
