...
public class ConfigCatalogTests
{
    ...
    public void TheEditableSurface_IsExactlyWhatItWasBeforeItBecameAProjection()
    {
        ...
        Assert.True(editable.Count == 108,
          ...
          + "Danger zone's server-side confirmation checks, 106 since v0.3.8.149 added "
          + "`auto_update` — which crossed WITHOUT this line being updated, so the figure recorded "
          + "here had already fallen one behind the surface it describes, which is the whole defect "
          + "this guard exists to catch — and 108 since v0.3.8.157 added "
          + "`knowledge_auto_study` and `knowledge_forager_endpoint` — the first because a labelled "
          + "switch IS the deliberate choice its file-only argument was protecting, the second "
